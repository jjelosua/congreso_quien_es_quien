var tpl = _.template("<div><a href='diputado/<%= id %>' target='_blank'><img class='lazy-load' dsrc='blank.gif' data-original='data/img/<%= id_diputado %>_<%= id_legislatura %>.jpg' alt='<%= nombre %>' title='<%= gr_nombre %>, Circunscripcion: <%= circ_nombre %>.'/></a><h2 style='text-align:left;'><a href='diputado/<%= id %>' target='_blank'><%= nombre %></a></h2><div class='details'>Partido: <%= partido %><br>Circunscripción: <%= circ_nombre %></div></div>");
