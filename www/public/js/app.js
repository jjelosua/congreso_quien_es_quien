var tpl = _.template("<div><a href='dip/<%= id %>'><img class='lazy-load' dsrc='blank.gif' data-original='data/img/<%= id_diputado %>_<%= id_legislatura %>.jpg' alt='<%= nombre %>' title='<%= gr_nombre %>, Circunscripcion: <%= circ_nombre %>.'/></a><h2><%= nombre %></h2><div class='details'>Partido: <%= partido %><br>Circunscripción: <%= circ_nombre %></div></div>");
...
$(function() {
    d3.csv("data/dip_congreso.csv", function(d) {return d;}
      ,function(error, rows) {
