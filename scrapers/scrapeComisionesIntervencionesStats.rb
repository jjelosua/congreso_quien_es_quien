...
QUERY_URL = 'oriIC=S&CMD=VERLST&FMT=INTTXLGE.fmt&DOCS=1-25&DOCORDER=FIFO&OPDEF=Y'
TARGET_URL = 'oriIC=S&CMD=VERLST&FMT=INTTXLGE.fmt&DOCS=1-25&DOCORDER=FIFO&OPDEF=Y'
#Headers for the output file
HEADERS = ["id_legislatura","id_comision","intervenciones","url"]   
...
CSV.foreach("#{INPUT_SUBDIR}/#{INPUT_FILE}") do |row|
  ...
  if results.length > 0
    if results.text =~ /Intervenciones\s+encontradas.*?(\d+)/
      intervenciones = $1.to_i unless $1.nil?
    end
  end
  ...
  data = [id_legislatura,id_comision,intervenciones,target_url]
